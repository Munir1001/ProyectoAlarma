...
const UserSectorAlarmPage: React.FC = () => {
  ...
  const [sectorUsuarioActual, setSectorUsuarioActual] = useState<string>("");
  ...
  useEffect(() => {
    // Obtener el sector del usuario actual y asignarlo al estado
    if (usuarioActual) {
      setSectorUsuarioActual(usuarioActual.Sector);
    }
  }, [usuarioActual]);
  ...
  const filteredAlarmas = alarmas.filter(
    (alarma) =>
      alarma.CedulaUsuario === usuarioActual?.Cedula ||
      usuarios.some(
        (usuario) =>
          usuario.Cedula === alarma.CedulaUsuario &&
          usuario.Sector === sectorUsuarioActual
      )
  );
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle className="ion-text-center">Alarmas de Mi Sector</IonTitle>
        </IonToolbar>
      </IonHeader>
...
export default UserSectorAlarmPage;
